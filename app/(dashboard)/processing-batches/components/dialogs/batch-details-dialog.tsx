...
export function BatchDetailsDialog({
  ...
  const displayedOverallStatus =
    batch?.latestStageSummary?.status || "NO_STAGES";
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2 text-sm p-3 border rounded-md bg-muted/20">
                    ...
                    <div>
                      <span className="text-xs text-muted-foreground block">
                        Total Sold from Batch:
                      </span>
                      <span className="font-semibold block">
                        {batch.totalQuantitySoldFromBatch.toFixed(2)} kg
                      </span>
                    </div>
                    <div>
                      <span className="text-xs text-muted-foreground block">
                        Net Available (latest stage):
                      </span>
                      <span className="font-bold text-green-600 block">
                        {batch.netAvailableQuantity.toFixed(2)} kg
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  batch.processingStages.length > 0 ? (
                    batch.processingStages.map(
                      (stage: ProcessingStageWithDetails) => (
                        <div
                          key={stage.id}
                          className="mt-2.5 border p-3 rounded-md bg-card shadow-sm"
                        >
                                  ...
                                  <TableBody>
                                    {stage.dryingEntries.map((d) => (
                                      <TableRow key={d.id}>
                                        ...
                                        <TableCell className="py-1">
                                          {d.moisturePercentage}%
                                        </TableCell>
                                        <TableCell className="text-right py-1">
                                          {d.currentQuantity.toFixed(2)}
                                        </TableCell>
                                      </TableRow>
                                    ))}
                                  </TableBody>
                                ...
                                <Table className="text-xs">
                                  ...
                                  <TableBody>
                                    {stage.sales.map((s) => (
                                      <TableRow key={s.id}>
                                        <TableCell className="py-1">
                                          {format(new Date(s.dateOfSale), "PP")}
                                        </TableCell>
                                        <TableCell className="text-right py-1">
                                          {s.quantitySold.toFixed(2)}kg
                                        </TableCell>
                                      </TableRow>
                                    ))}
                                  </TableBody>
                ...
                <div>
                  <h3 className="text-md font-semibold mb-1.5">
                    All Sales From Batch ({batch.sales?.length || 0})
                  </h3>
                  <Separator />
                        ...
                        <TableBody>
                          {batch.sales.map((s) => (
                            <TableRow key={s.id}>
                              ...
                              <TableCell className="py-1.5">
                                P{s.processingStage.processingCount}
                              </TableCell>
                              <TableCell className="text-right py-1.5">
                                {s.quantitySold.toFixed(2)}
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
