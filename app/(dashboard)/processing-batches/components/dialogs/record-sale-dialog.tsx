...
interface ProcessingStageInfoForSaleDialog {
  ...
  status:
    | typeof ProcessingStageStatus
    | "SOLD_OUT"
    | "NO_STAGES"
    | "IN_PROGRESS"
    | "FINISHED"
    | "CANCELLED"; // For display
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            Record Sale from Batch {batchCode} - Stage P
            {processingStage.processingCount}
          </DialogTitle>
          <DialogDescription>
            Stage Yield:{" "}
            {(processingStage.quantityAfterProcess || 0).toFixed(2)}kg.
            Available from this stage: {availableForSaleFromStage.toFixed(2)}kg.
