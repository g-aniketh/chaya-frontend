...
import { z } from "zod";
import { type CreateProcessingStageInput as BackendCreateProcessingStageInputType } from "@ankeny/chaya-prisma-package/client";
...
export function StartNextStageDialog({
  ...
  const onSubmit = async (data: NextStageFormValues) => {
    ...
    try {
      ...
      await axios.post(`/api/processing-stages`, payload, {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Stage P{previousProcessingCount} finished with a yield of{" "}
            {previousStageYield.toFixed(2)}kg. The new stage P
            {nextProcessingCount} will start with the quantity remaining after
            any sales from P{previousProcessingCount}.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  <FormLabel>
                    Date of Processing (P{nextProcessingCount})
                  </FormLabel>
                  <Popover>
