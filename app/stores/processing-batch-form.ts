...
const initialFirstStageDetailsState: Partial<
  CreateProcessingBatchInput["firstStageDetails"]
> = {
  processMethod: "wet",
...
export const useProcessingBatchFormStore = create<ProcessingBatchFormState>(
  (set, get) => ({
    ...initialCoreState,
    ...
    goToNextStep: () => {
      ...
      if (activeStep === "selectCriteria")
        ...
        set({ activeStep: "firstStageDetails" });
      else if (activeStep === "firstStageDetails")
        set({ activeStep: "review" });
    },
    goToPreviousTab: () => {
      const { activeStep } = get();
    ...
    setAvailableProcurements: (procurements) =>
      set({ availableProcurements: procurements }),
        ...
        if (isSelected) {
          ...
          if (newSelectedIds.length === 0) {
            newFilterCriteriaLocked = !!(
              state.initialCrop && state.initialLotNo
            );
          ...
          if (
            !state.filterCriteriaLocked ||
            state.selectedProcurementIds.length === 0
          ) {
