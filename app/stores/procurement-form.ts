...
export const procurementFullFormSchema = basicInfoSchemaForStore.merge(
  detailsSchemaForStore
);
export type ProcurementFullFormValues = z.infer<
...
interface ProcurementFormState {
  ...
  mode: "add" | "edit";
  initializeForm: (
    initialData: ProcurementWithRelations | null | undefined,
    mode: "add" | "edit"
  ) => void;
}

export const useProcurementFormStore = create<ProcurementFormState>(
  (set, get) => ({
    activeTab: "basic",
    ...
    goToNextTab: async () => {
      ...
      if (form) {
        ...
        if (fieldsToValidate.length > 0) {
          ...
          if (!isValid) {
            toast.error(
              "Please correct the errors on the current tab before proceeding."
            );
            return;
          }
        }
